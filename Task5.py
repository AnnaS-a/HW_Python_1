# Задача 5. Напишите программу, которая принимает на вход координаты двух точек и 
# находит расстояние между ними в 2D пространстве. 
# Пример:   A (3,6); B (2,1) -> 5,09         A (7,-5); B (1,-1) -> 7,21

x1 = int(input('Введите значение x точки A: '))
y1 = int(input('Введите значение y точки A: '))
x2 = int(input('Введите значение x точки B: '))
y2 = int(input('Введите значение y точки B: '))

from math import sqrt
distance = round(sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)), 2)
print(f'расстояние между А({x1}, {y1}) и B({x2}, {y2}) равно {distance}')

